...
async function fetchStockData() {
    ...
    try {
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
                const url = `https://polygon-api-worker.raheimbbailey.workers.dev/?startDate=${dates.startDate}&endDate=${dates.endDate}&ticker=${ticker}`
...
async function fetchReport(data) {
    ...
    try {
        const url = "https://openai-api-worker.raheimbbailey.workers.dev/"

        const response = await fetch(url, {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(messages)})
